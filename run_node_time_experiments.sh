    ...
    for technique_name in "${technique_names[@]}"; do
        ...
        if [ "$technique_name" == "combined" ]; then
            ...
            for policy in "${policies[@]}"; do
                ...
                python main.py run_mode=sweep logger.mode=online explainer=$technique_name scheduler.policy=$policy dataset=$data model=$model workers=1 num_agents=1 max_samples=80 project="COMBINEX-TIME" logger.config=time_sweep task=node
            done
        else
            echo "Running with technique: $technique_name"
            
            # Run the Python command without scheduler.policy
            python main.py run_mode=sweep logger.mode=online explainer=$technique_name dataset=$data model=$model  workers=1 num_agents=1 max_samples=80 project="COMBINEX-TIME" logger.config=time_sweep task=node
        fi
    done
done