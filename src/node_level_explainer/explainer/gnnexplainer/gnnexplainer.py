...
class GNNExplainerWrap(ExplainerABC):
    ...
    def explain(self, graph: Data, oracle: nn.Module) -> dict:
        ...
        original_out = oracle(graph.x, graph.edge_index)
        ...
        counterfactual_out = oracle(counterfactual_data.x, counterfactual_data.edge_index)
        ...
        print(counterfactual_out[graph.new_idx], graph.targets[graph.new_idx])
        # Check if the counterfactual prediction matches the target and has a lower loss
        if (counterfactual_pred[graph.new_idx] == graph.targets[graph.new_idx]):
            counterfactual = build_counterfactual_graph(x=node_features, edge_index=graph.edge_index, graph=graph, oracle=oracle, output_actual=counterfactual_out)
