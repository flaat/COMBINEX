...
class EdgePerturber(Perturber):
    ...
    def forward(self) -> Tensor:       
        ...
        if self.cfg.task.name.lower() == "node":
            args = {"x":self.graph_sample.x, "edge_index":self.graph_sample.edge_index, "edge_weights":F.sigmoid(self.P_x)}
        elif self.cfg.task.name.lower() == "graph":
            args = {"x":self.graph_sample.x, "edge_index":self.graph_sample.edge_index, "batch":self.graph_sample.batch,  "edge_weights":F.sigmoid(self.P_x)}
    ...
    def forward_prediction(self):
        ...
        if self.cfg.task.name.lower() == "node":
            args = {"x":self.graph_sample.x, "edge_index":self.graph_sample.edge_index, "edge_weights":P_x_discrete}
        elif self.cfg.task.name.lower() == "graph":
            args = {"x":self.graph_sample.x, "edge_index":self.graph_sample.edge_index, "batch":self.graph_sample.batch,  "edge_weights":P_x_discrete}
    ...
    def loss(self, graph, output, y_node_non_differentiable):
        ...
        y_node_oracle_original = graph.y[graph.new_idx] if self.cfg.task.name.lower() == "node" else graph.y # Original ground truth
        y_target = graph.targets.unsqueeze(0) if self.cfg.task.name.lower() == "graph" else graph.targets  # Counterfactual target
        
        # Calculate if the prediction is already flipped
        y_node_non_differentiable = y_node_non_differentiable[graph.new_idx] if self.cfg.task.name.lower() == "node" else y_node_non_differentiable
