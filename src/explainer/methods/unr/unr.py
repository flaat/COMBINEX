...
from src.utils.utils import ExplanationContext, build_counterfactual_graph
...
class UNRExplainer(ExplainerABC):
    ...
    def explain(self, graph: Data, oracle, **kwargs) -> dict:
        ...
        context = ExplanationContext(y_pred_new_actual=torch.argmax(output_counterfactual, dim=1),
                           ...
                           graph=graph,
                           oracle=oracle,
                           output_actual=output_counterfactual,
                           device=self.device)
