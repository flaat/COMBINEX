...
class CFFExplainer(ExplainerABC):
    ...
    def explain(self, graph: Data, oracle) -> dict:
        ...
        explainer = ExplainModelNodeMulti(cfg=self.cfg,
            graph=graph,
            base_model=oracle
        )
        ...
        for _ in range(self.epochs):
            ...
            if self.cfg.task.name.lower() == "node":
                ...
                pred1, pred2 = pred1[graph.new_idx], pred1[graph.new_idx]
                target = graph.y_ground[graph.new_idx]
            else:
                pred1, pred2 = pred1.squeeze(0), pred2.squeeze(0)
                target = graph.y_ground.long()
            loss = explainer.loss(pred1, pred2, target, self.gam, self.lam, self.alp)
            loss.backward()
            optimizer.step()
            ...
            if self.cfg.task.name.lower() == "node":
                args = {"x":graph.x, "edge_index":masked_adj[0], "edge_weights":masked_adj[1]}
                target = graph.targets[graph.new_idx]
            elif self.cfg.task.name.lower() == "graph":
                args = {"x":graph.x, "edge_index":masked_adj[0], "batch":graph.batch,  "edge_weights":masked_adj[1]}
                target = graph.targets.unsqueeze(0)  # Assuming a single graph in batch
            
            out = oracle(**args)
            edge_index_counterfactual = masked_adj[0][:, masked_adj[1] > 0.5]
            y_new = torch.argmax(out, dim=1)
            
            
            context = ExplanationContext(y_pred_new_actual=y_new,
...
class ExplainModelNodeMulti(torch.nn.Module):
    ...
    def __init__(self, cfg, graph, base_model):
    ...
    def forward(self):
        ...
        if self.cfg.task.name.lower() ==  "graph":
            args = {"x":self.graph.x, "edge_index":masked_adj[0], "edge_weights":masked_adj[1], "batch":self.graph.batch}
            
        else:
            args = {"x":self.graph.x, "edge_index":masked_adj[0], "edge_weights":masked_adj[1]}
        
        S_f = self.base_model(**args)  # Counterfactual graph
        
        if self.cfg.task.name.lower() ==  "graph":
            args = {"x":self.graph.x, "edge_index":masked_adj[0], "batch":self.graph.batch}
            
        else:
            args = {"x":self.graph.x, "edge_index":masked_adj[0] }
        S_c = self.base_model(**args)  # Original graph
        return S_f, S_c
